...
logFile = "D:\ONGC RAM proj\check_logs_result.txt"
...
Function ParseResult(resp)
    ...
    ElseIf InStr(resp, "generated_sql") > 0 Then
        ...
        If sqlPos > 0 Then
            sqlSnip = Mid(resp, sqlPos + 16, 300)
            Dim endQ
            endQ = InStr(sqlSnip, """")
    ...
    ElseIf InStr(resp, "detail") > 0 Then
        msg = "FAIL: " & Left(resp, 400)
    Else
        msg = "RESP: " & Left(resp, 400)
    End If
    ParseResult = msg
End Function

Log "ONGC Explicit Test v8 - " & Now()
Log "URL: " & baseUrl & "/api/query | Auth: Bearer <token>"
Log "(Using explicit table names to eliminate AI ambiguity)"
Log "============================================================"
Log ""
